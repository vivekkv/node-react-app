...
import { getCapacities } from 'services/capacity';
import { INPUT_CHANGE, SET_FORM, INIT_MODULE, INITALIZE, SUBMIT_ITEM, DELETE_ITEM, EDIT_ITEM, CLEAR_FORM } from 'constants/admin/capacity';
import { format, log, debug } from 'util';

const getStateData = (state) => Map(state.admin.capacity);
    ...
    while (true) {
        ...
        try {
            ...
            if (response.completed && response.data.success) {
                ...
                let list = formData.get("capacityList").toArray();
                ...
                yield put(reduxAction(SET_FORM, { 'data': { 'capacityList': List(list) } }))
