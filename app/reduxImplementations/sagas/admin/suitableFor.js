...
import { getSuitableFor } from 'services/suitableFor';
import { INPUT_CHANGE, SET_FORM, INIT_MODULE, INITALIZE, SUBMIT_ITEM, DELETE_ITEM, EDIT_ITEM, CLEAR_FORM } from 'constants/admin/suitableFor';
import { format, log, debug } from 'util';

const getStateData = (state) => Map(state.admin.suitableFor);
...
function* onSubmit() {
	...
	while (true) {
		...
		try {
			...
			let response = yield call(callApi, "suitableFor", {
				method: 'POST',
				body: JSON.stringify({
					'id': formData.get("id"),
			...
			if (response.completed && response.data.success) {
				...
				let list = formData.get("suitableForList").toArray();
				...
				yield put(reduxAction(SET_FORM, { 'data': { 'suitableForList': List(list) } }))
